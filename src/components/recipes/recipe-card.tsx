...
const Footer: React.FC<{ recipe: SafeRecipe }> = ({ recipe }) => {
    ...
    <CardFooter className="flex flex-col items-start p-3">
      <div className="flex justify-between items-center w-full">
        ...
        <div className="flex flex-row gap-[2px] items-center justify-end basis-2/12">
          <FaStar size={14} />
          <div className="text-md font-semibold">{averageRating || 0}</div>
        </div>
      </div>
      <div className="flex items-center space-x-2 mt-2 leading-7 text-muted-foreground">
        <input
